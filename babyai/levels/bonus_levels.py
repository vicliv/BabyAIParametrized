import gym
import json
from babyai.minigrid.minigrid import Key, Ball, Box, IDX_TO_COLOR, IDX_TO_OBJECT
...
class Level_JSON(RoomGridLevel):
    ...
    def gen_mission(self):
        self.place_agent(self.data['agent']['room'][0], self.data['agent']['room'][1], False, pos = np.array(self.data['agent']['pos']), dir = self.data['agent']['dir'])
        
        for object in self.data['objects']:
            self.add_object(object['i'], object['j'], kind=object['type'], color=object['color'], pos=np.array(object['pos']))

        for door in self.data['doors']:
            self.add_door(door['i'], door['j'], door_idx = door['idx'], color = door['color'], locked = door['locked'], pos= door['pos'])
...
class Level_parametrized(LevelGen):
    ...
    def gen_mission(self):
        self.place_agent(self.data[3], self.data[4], False, pos = np.array([self.data[5], self.data[6]]), dir = self.data[7])
        
        objs = []
        for i in range(8, 116, 6):
            if self.data[i] == -1:
                continue
            self.add_object(self.data[i], self.data[i+1], kind=IDX_TO_OBJECT[self.data[i+2]], color=IDX_TO_COLOR[self.data[i+3]], pos=np.array([self.data[i+4], self.data[i+5]]))
        ...
        for i in range(116, 188, 6):
            if self.data[i] == -1:
                continue
            self.add_door(self.data[i], self.data[i+1], door_idx = self.data[i+2], color = IDX_TO_COLOR[self.data[i+3]], locked = self.data[i+4], pos = self.data[i+5])
